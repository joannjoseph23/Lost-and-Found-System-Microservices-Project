...
export function AdminMatches() {
  ...
  const maxScore = useMemo(() => {
    const m = Math.max(0, ...matches.map(m => Number(m.score) || 0));
    return m || 1;
  }, [matches]);
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-4">
        {matches.map((m) => {
          ...
          const img = found?.imagePath ? `${API_BASE}${found.imagePath}` : null;

          return (
            <Card key={m.id} className="p-4">
              <div className="flex gap-4">
                {/* IMAGE */}
                <div className="w-44 h-32 bg-muted rounded overflow-hidden shrink-0">
                  {img ? (
                    <img
                      src={img}
